    ...
    <BaseLayer ref="baselayer">
      ...
      <template v-slot:default>
        ...
        <div class="container">
          <!-- 搜索城市 -->
          <ul>
            <li v-for="(item,index) in searchlist" :key="index" style="font-size:20px;text-align:center;line-height:30px">{{item.name}}</li>
          </ul>
          <!-- 定位 -->
...
export default {
  ...
  methods: {
    // 模糊搜索
    search(){
        for(var key in this.cityDatas){
          if(key!="hot"){
                for(var i=0;i<this.cityDatas[key].length;i++){
              // console.log(this.cityDatas[key][i]);
              this.searchDateList.push(this.cityDatas[key][i]);
            }
          }
        }
        this.searchlist=this.searchDateList.filter(ele=>ele.name.includes(this.searchvalue));//ele.name.indexOf(this.searchvalue)!=-1;
  ...
  directives: {
    ...
    touch(el, binding, vnode) {
      ...
      function updateVm(endY, baseTop, baseHeight, vm) {
        ...
        if (dom) {
          // vm.$root.$BScroll.scrollToElement也行
