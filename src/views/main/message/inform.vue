<template>
  <div class="inform-main">
    ...
    <div class="input-box">
      <input type="text" placeholder="请输入想说的话" v-model.trim="myWord" @keyup.enter="sendMsg"/>
...
export default {
  ...
  methods: {
    ...
    getSiZhi() {  //  思知机器人
      var that = this;
      axios.get('https://api.ownthink.com/bot', {
        params: {
          spoken: `${this.myWord}`
        }
      })
      .then(response => {
        var obj = {
          text: response.data.data.info.text,
          time: new Date().toLocaleString(),
          type: "other"
        }
    ...
    getTuling() { //  图灵机器人接口 一天只能请求 5 次, 超出显示 请求次数超限！
      var params = {
        reqType: 0,
        perception: {
          inputText: {
    ...
    resizeScrollY() {
      ...
      this.$nextTick(function() {
        ...
        lastDom = this.$refs.wordWrapper[this.$refs.wordWrapper.length-1];
        step = lastDom.offsetHeight;
        //  超出对话栏可视区域
        if (maxY != 0) {
          console.log(step, lastDom.offsetHeight);
          this.$refs.baseLayer.$scroll.scrollBy(0, -step, 400, "easing");
        }
      })
    }
  },
