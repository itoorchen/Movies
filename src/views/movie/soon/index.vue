<template>
    <div class="soon-main">
        ...
        <swiper :options="dateOption" class="date-swiper" ref="dateItem">
            <swiper-slide class="date-swiper-slide" v-for="item in dateList" :key="item.id">
                <div :class="`date${activeClass==item.id?' active':''}`" @click="addAct(item.id)">
                     {{item.text}}
                </div>
            </swiper-slide>
        </swiper>
...
export default {
    ...
    methods:{
        ...
        foo(e){
            ...
            this.$refs.dateItem.swiper.slideTo(index, 1000, false);
        }
    },
    created(){
